package com.chinapost.devp.generate.web.api.chart;
...
public interface MetaChartAPI {
     ...
     * 新增【树】
     */
    @ApiOperation(value="新增【树】")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "addDTO", dataType = "PieAddDTO", value = "新增【树】参数", paramType = "body"),
    })
    ResponseEntity<TreeVO> saveTree(TreeAddDTO addDTO) throws Exception;

    /**
     * 查看【树表】详情
     */
    @ApiOperation(value="查看【树表】详情")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "chartId", dataType = "int", value = "【树】id", paramType = "path"),
    })
    ResponseEntity<TreeVO> showTree(Integer chartId);
