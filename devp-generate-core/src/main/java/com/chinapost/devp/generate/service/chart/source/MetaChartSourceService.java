package com.chinapost.devp.generate.service.chart.source;
...
import com.chinapost.devp.generate.dao.chart.MetaChartSourceDAO;
...
import com.chinapost.devp.generate.pojo.dto.chart.source.item.*;
...
import com.chinapost.devp.generate.pojo.po.chart.source.item.*;
...
import com.chinapost.devp.generate.pojo.vo.chart.source.item.*;
import com.chinapost.devp.generate.service.MetaProjectService;
import com.chinapost.devp.generate.service.chart.source.item.*;
...
public class MetaChartSourceService {
    ...
    public MetaChartSourcePO saveWithItems(MetaChartSourceWithItemsAddDTO addDTO) {
        ...
        // 保存节点数据
         NodeAddDTO treeAddDTO = addDTO.getTreeNodes();
        if (ObjectUtil.isNotEmpty(treeAddDTO)) {
            // 节点id
            DetailColumnAddDTO id = treeAddDTO.getId();
            if (ObjectUtil.isNotEmpty(id)) {
                id.setSourceId(sourceId);
                id.setProjectId(projectId);
                detailColumnService.doSave(id);
            }
            // 父节点id
            DetailColumnAddDTO parentId = treeAddDTO.getParentId();
            if (ObjectUtil.isNotEmpty(parentId)) {
                parentId.setSourceId(sourceId);
                parentId.setProjectId(projectId);
                detailColumnService.doSave(parentId);
            }
            // 父节点ids
            DetailColumnAddDTO parentIds = treeAddDTO.getParentIds();
            if (ObjectUtil.isNotEmpty(parentIds)) {
                parentIds.setSourceId(sourceId);
                parentIds.setProjectId(projectId);
                detailColumnService.doSave(parentIds);
            }
            // 节点名称
            DetailColumnAddDTO label = treeAddDTO.getLabel();
            if (ObjectUtil.isNotEmpty(label)) {
                label.setSourceId(sourceId);
                label.setProjectId(projectId);
                detailColumnService.doSave(label);
            }
            // 排序
            DetailColumnAddDTO sort = treeAddDTO.getSort();
            if (ObjectUtil.isNotEmpty(sort)) {
                sort.setSourceId(sourceId);
                sort.setProjectId(projectId);
                detailColumnService.doSave(sort);
            }
            // 叶子节点
            DetailColumnAddDTO leaf = treeAddDTO.getLeaf();
            if (ObjectUtil.isNotEmpty(leaf)) {
                leaf.setSourceId(sourceId);
                leaf.setProjectId(projectId);
                detailColumnService.doSave(leaf);
            }
        }
        metaProjectService.updateProject(project);
        return metaChartSource;
